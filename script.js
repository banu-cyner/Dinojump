...
let gameLoopId = null;
let isGameOver = false; // Flag penanda game selesai
let score = 0;
let highScore = 0;
let gameSpeed = 4;
let gravity = 0.6;
...
// Init Game Loop
function initGame() {
  isGameOver = false; // Reset state game over
  score = 0;
  gameSpeed = 4;
  obstacles = [];
  spawnTimer = 0;
...
function update() {
  if (isGameOver) return; // Hentikan kalkulasi jika game over
...
function gameLoop() {
  if (isGameOver) return; // Hentikan loop animasi sepenuhnya
...
function gameOver() {
  isGameOver = true; // Tandai bahwa permainan telah berhenti
