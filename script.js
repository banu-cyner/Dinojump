...
function spawnObstacle() {
  let height = Math.floor(Math.random() * 20) + 25; // Tinggi rintangan bervariasi
  obstacles.push({
    x: canvas.width,
...
function update() {
  // Update Runner (Gravitasi & Posisi)
  runner.dy += gravity;
  runner.y += runner.dy;
  ...
  if (Math.floor(score) % 100 === 0 && score > 0) {
    gameSpeed += 0.005; // Bertambah cepat perlahan
  }
  ...
  // Update & Collsion Check Obstacles
  ...
  fetch(SCRIPT_URL)
    ...
    .then((data) => {
      ...
      if (!data || data.length === 0) {
        leaderboardList.innerHTML = '<li class="loading">Belum ada skor tercatat.</li>';
        return;
      }
